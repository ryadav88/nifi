...
class UniversePackageBuilder(object):
    ...
    def _get_myfw_mapping_for_content(self, orig_content):
        '''Returns a myfw mapping (dict) for the following cases:
        ...
        - Custom environment params like 'MYFW_SOME_PARAM' which maps to '{{some-param}}'
        '''
        # default myfw values (may be overridden via eg MYFW_PACKAGE_VERSION envvars):
        now = time.time()
        myfw_mapping = {
        ...
        # look for any 'sha256:filename' myfw params, and get shas for those.
        # this avoids calculating shas unless they're requested by the myfw.
        for shafilename in re.findall('{{sha256:(.+?)}}', orig_content):
            ...
            myfw_mapping['sha256:{}'.format(
                shafilename)] = self._calculate_sha256(shafilepath)

        # import any custom MYFW_SOME_PARAM environment variables:
        for env_key, env_val in os.environ.items():
            if env_key.startswith('MYFW_'):
                # 'MYFW_SOME_KEY' => 'some-key'
                myfw_mapping[env_key[len('MYFW_'):].lower().replace(
                    '_', '-')] = env_val

        return myfw_mapping

    def _apply_templating_to_file(self, filename, orig_content):
        myfw_mapping = self._get_myfw_mapping_for_content(orig_content)
        new_content = orig_content
        for myfw_key, myfw_val in myfw_mapping.items():
            new_content = new_content.replace('{{%s}}' % myfw_key,
                                              myfw_val)
        ...
        myfw_keys = list(myfw_mapping.keys())
        myfw_keys.sort()
        for key in myfw_keys:
            logger.info('  {{%s}} => %s' % (key, myfw_mapping[key]))
