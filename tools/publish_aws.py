...
class AWSPublisher(object):
    ...
    def __init__(
        ...
        s3_bucket = os.environ.get('S3_BUCKET', 'ryv-bucket')
...
def print_help(argv):
    logger.info('Syntax: {} <package-name> <myfw-package-dir> [artifact files ...]'.format(argv[0]))
    ...
    logger.info('In addition, environment variables named \'MYFW_SOME_PARAMETER\' will be inserted against the provided package myfw (with params of the form \'{{some-parameter}}\')')
...
def main(argv):
    ...
    # the package name:
    package_name = argv[1]
    # local path where the package myfw is located:
